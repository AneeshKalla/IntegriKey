...
@app.route('/last-overlayed-image')
def last_overlayed_image():
    try:
        last_result = results[-1]  # Get the last element in results
        last_image = last_result[1]
        if last_image is not None:
            filename = 'path/to/last_image.png'  # Save last_image to a file or use an existing filename
            last_image.save(filename)  # Assuming last_image is a PIL Image or similar; adjust saving accordingly
